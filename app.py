...
def query_ollama(prompt, system_prompt="", temperature=0.3, max_retries=2):
    payload = {
        "model": OLLAMA_MODEL,
        "prompt": prompt,
...
"""
    return query_ollama(prompt, system_prompt=SRS_SYSTEM_PROMPT, temperature=0.3)
...
"""
    return query_ollama(prompt, system_prompt=SDD_SYSTEM_PROMPT, temperature=0.3)
...
"""
    return query_ollama(prompt, system_prompt=SECTION_REGEN_PROMPT.format(doc_type=doc_type.upper()))
...
"""
    return query_ollama(prompt, temperature=0.1)
...
def api_generate():
    ...
    if doc_type == "srs":
        ...
        # For SDD, use existing SRS content
...
if __name__ == "__main__":
    ...
    print(f"  Ollama Status: {'CONNECTED' if check_ollama() else 'NOT RUNNING'}")
